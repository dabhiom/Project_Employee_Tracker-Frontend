...
const normalise = (e) => ({
  ...
  lastName:
    e.lastName || (e.fullName || "").split(" ").slice(1).join(" ") || "",
  employeeId: e.employeeId || e.empId || e._id,
  dateOfJoining: e.doj || e.dateOfJoining || e.employeeStartDate || "",

  designation:
    e.designationId?.designationName || e.designation || e.designationId || "—",

  department:
    e.departmentId?.departmentName || e.department || e.departmentId || "—",

  baseLocation:
    e.baseLocationId?.locationName || e.baseLocation || e.baseLocationId || "—",

  currentLocation:
    e.currentLocationId?.locationName ||
    e.currentLocation ||
    e.currentLocationId ||
    "—",

  reportingManager:
    e.reportingManagerId?.fullName ||
    e.reportingManager ||
...
const AvatarBox = ({ firstName, lastName, size = 34, fontSize = "0.8rem" }) => {
  ...
  const idx =
    ((firstName?.[0] || "A").charCodeAt(0) +
      (lastName?.[0] || "A").charCodeAt(0)) %
    colors.length;
  return (
    <Box
      sx={{
...
const InfoRow = ({ icon: Icon, label, value }) => (
  <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
    ...
    <Typography variant="caption" fontWeight={600} color="#1a2740" noWrap>
      {value || "—"}
    </Typography>
  </Box>
);
...
export default function EmployeesPage() {
  ...
  const handleSave = async (formData) => {
    try {
      if (editData) {
        const body = {
          _id: editData._id,
          employeeId: formData.employeeId || formData.empId || "",
          ...
          reportingManagerId: formData.reportingManager || "",
          employeeStatus:
            STATUS_CODE[formData.employeeStatus] ??
            formData.employeeStatus ??
            "0",
          doj: formData.dateOfJoining || formData.employeeStartDate || "",
          ...
          baseLocationId: formData.baseLocation || "",
          currentLocationId: formData.currentLocation || "",
          passportAvailable: formData.passportAvailable || false,
        };
        ...
        setEmployees((p) =>
          p.map((e) => (e._id === editData._id ? updated : e)),
        );
        ...
        const body = {
          ...
          lastName: formData.lastName || "",
          email: formData.email || "",
          password: formData.password || "Welcome@123", // default password for new employees
          role: formData.role || "4",
          designationId: formData.designation || "",
          departmentId: formData.department || "",
          reportingManagerId: formData.reportingManager || "",
  ...
  const stats = [
    ...
    },
    {
      ...
      value: loading
        ? "…"
        : employees.filter((e) => e.employeeStatus === "0").length,
      icon: CheckCircle,
    ...
    },
    {
      ...
      value: loading
        ? "…"
        : employees.filter((e) => e.employeeStatus === "1").length,
      ...
      >
        ...
        <Button
          ...
          sx={{
            ...
            "&:hover": {
              bgcolor: "#15305a",
              boxShadow: "0 6px 18px rgba(26,60,110,0.35)",
            },
          }}
        >
      ...
      >
          ...
          InputProps={{
            startAdornment: (
              <InputAdornment position="start">
                <Search fontSize="small" sx={{ color: "#8faabf" }} />
              </InputAdornment>
            ),
          }}
        />
        ...
        <TextField
          ...
          InputProps={{
            startAdornment: (
              <InputAdornment position="start">
                <FilterList fontSize="small" sx={{ color: "#8faabf" }} />
              </InputAdornment>
            ),
          }}
        >
        ...
        <TextField
          ...
          InputProps={{
            startAdornment: (
              <InputAdornment position="start">
                <Person fontSize="small" sx={{ color: "#8faabf" }} />
              </InputAdornment>
            ),
          }}
        >
        ...
        <Box sx={{ ml: { sm: "auto" }, display: "flex", alignItems: "center" }}>
          ...
          >
            Showing{" "}
            <strong style={{ color: "#1a3c6e" }}>{filtered.length}</strong> of{" "}
            {employees.length}
          </Typography>
        </Box>
      </Paper>
            ...
            filtered.map((row) => (
              ...
              >
                <Box
                  sx={{
                    background:
                      "linear-gradient(135deg, #1a3c6e 0%, #1e4d8c 100%)",
                    px: 2,
                    py: 1.5,
                    display: "flex",
                    justifyContent: "space-between",
                ...
                >
                  <Box>
                    ...
                    <Typography fontSize="0.72rem" color="#93c5fd">
                      {row.employeeId}
                    </Typography>
                  </Box>
                  <Chip
                ...
                >
                  <InfoRow
                    icon={Business}
                    label="Role"
                    value={`${row.designation || "—"} • ${row.department || "—"}`}
                  />
                ...
                >
                  <Button
                    size="small"
                    startIcon={
                      <Visibility sx={{ fontSize: "13px !important" }} />
                    }
                ...
                {filtered.length === 0 ? (
                  ...
                  filtered.map((row, idx) => (
                    ...
                    >
                      ...
                      {/* Department */}
                      <TableCell
                        sx={{ fontSize: "0.84rem", py: 1.75, color: "#4a5e78" }}
                      >
                        {row.department || "—"}
                      </TableCell>
                      ...
                      <TableCell sx={{ py: 1.75 }}>
                        ...
                        <Typography fontSize="0.72rem" color="#8faabf" mt={0.2}>
                          {row.phone || "—"}
                        </Typography>
                      </TableCell>
          ...
          {filtered.length > 0 && (
            ...
            >
              <Typography fontSize="0.75rem" color="text.secondary">
                Showing{" "}
                <strong style={{ color: "#1a3c6e" }}>{filtered.length}</strong>{" "}
          ...
          <Box>
            ...
            >
              ...
              <Box sx={{ flex: 1, minWidth: 0 }}>
                ...
                <Typography
                  fontSize="0.8rem"
                  sx={{ color: "rgba(255,255,255,0.75)", mt: 0.2 }}
                >
                  {detailData.designation}
                </Typography>
                <Typography
                  fontSize="0.72rem"
                  sx={{ color: "rgba(255,255,255,0.5)", mt: 0.1 }}
                >
            ...
            >
              <MasterDetailsModal.DetailItem
                label="Full Name"
                value={`${detailData.firstName} ${detailData.lastName}`}
              />
              <MasterDetailsModal.DetailItem
                label="Employee ID"
              ...
              <MasterDetailsModal.DetailItem
                label="Gender"
                value={detailData.genderLabel}
              />
              <MasterDetailsModal.DetailItem
                label="Date of Birth"
                value={fmtDate(detailData.dateOfBirth)}
              />
              <MasterDetailsModal.DetailItem
                label="Home Town"
                value={detailData.homeTown || "—"}
              />
            </Box>
            ...
            >
              <MasterDetailsModal.DetailItem
                ...
                value={fmtDate(detailData.dateOfJoining || detailData.doj)}
              />
              <MasterDetailsModal.DetailItem
                label="Status"
              ...
              <MasterDetailsModal.DetailItem
                ...
                value={
                  detailData.overallExperience
                    ? `${detailData.overallExperience} yrs`
                    : "—"
                }
              />
              <MasterDetailsModal.DetailItem
                ...
                value={
                  detailData.relevantExperience
                    ? `${detailData.relevantExperience} yrs`
                    : "—"
                }
              />
            </Box>
            ...
            >
              ...
              <MasterDetailsModal.DetailItem
                ...
                value={detailData.reportingManager}
              />
              <MasterDetailsModal.DetailItem
                label="Work Mode"
                value={detailData.workMode}
              />
              <MasterDetailsModal.DetailItem
                label="Base Location"
                value={detailData.baseLocation}
              />
              <MasterDetailsModal.DetailItem
                label="Current Location"
                value={detailData.currentLocation}
              />
            </Box>
          </Box>
        )}
...
function SectionHead({ children }) {
  return (
    <Box sx={{ mb: 1.5 }}>
      <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
        <Box
          sx={{ width: 3, height: 13, bgcolor: "#1a3c6e", borderRadius: 4 }}
        />
        <Typography
          sx={{
            fontSize: "0.65rem",
